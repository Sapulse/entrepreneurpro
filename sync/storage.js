...
function securizeData(d) {
  ...
  if(!Array.isArray(d.recurringRevenues)) d.recurringRevenues = [];
  if(!Array.isArray(d.recurringOccurrences)) d.recurringOccurrences = [];
...
function saveData(d) {
  ...
  (async () => {
    ...
    const entityFns = [
      ...
      typeof syncRecurringToTable     === 'function' ? syncRecurringToTable(d.recurringRevenues   || []) : null,
      typeof syncRecurringOccurrencesToTable === 'function' ? syncRecurringOccurrencesToTable(d.recurringOccurrences || []) : null,
    ...
    // _localVersion mis à jour AVANT le ping pour que la détection self-update fonctionne
    // (realtime peut arriver avant que le .then() ait mis à jour la variable)
    const newVersion = (_localVersion > 0 ? _localVersion : 0) + 1;
    _localVersion = newVersion; // anticipé — évite le rechargement parasite depuis realtime
